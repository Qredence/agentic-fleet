...
import { ConnectionStatusIndicator } from "@/components/features/shared";
import { PromptSuggestion } from "@/components/ui/custom/prompt-suggestion";
import { TextShimmer } from "@/components/ui/custom/text-shimmer";
...
import { ArrowDown, Loader2, Search, Target, TrendingUp } from "lucide-react";
...
// Lazy load heavy Plan component (only loads when plan is displayed)
import {
  Plan,
  PlanContent,
  PlanHeader,
...
export const ChatContainer = ({
  ...
  }, [displayMessages, timeFormatter]);

  const filteredPendingApprovals = useMemo(() => {
    return pendingApprovals.filter(
      (approval) =>
        approvalStatuses[approval.requestId]?.status !== "completed",
    );
  }, [pendingApprovals, approvalStatuses]);
    ...
    async (
      ...
      try {
        ...
        toast({
          ...
          description:
            err instanceof Error ? err.message : "Unable to submit approval",
          variant: "destructive",
        });
      }
    },
  ...
  const queueSummary = useMemo(() => {
    ...
    const phaseLabel =
      normalizedPhase.charAt(0).toUpperCase() +
      normalizedPhase.slice(1).toLowerCase();
  ...
  const logoSrc = isDarkMode ? "/logo-darkmode.svg" : "/logo-lightmode.svg";
    ...
    >
      ...
      <div className="flex w-full flex-1" style={contentStyle}>
        <div className="mx-auto flex w-full max-w-[800px] flex-1 flex-col pb-32">
              ...
              {filteredPendingApprovals.map((approval) => (
                <div key={approval.requestId} className="py-2">
                  <ApprovalPrompt
                    ...
                    onApprove={(options) =>
                      handleApprove(approval.requestId, options)
                    }
                    onReject={(reason) =>
                      handleReject(approval.requestId, reason)
                    }
                  />
                </div>
              ))}
          ...
          <div
            className="fixed bottom-[24px] left-0 right-0 z-10 px-4"
              ...
              {displayMessages.length === 0 && (
                <div className="flex flex-wrap justify-center items-center gap-2 py-2">
                  <PromptSuggestion
                    ...
                    onClick={() =>
                      handleSendMessage("Analyze quarterly performance")
                    }
                  >
                  ...
                  <PromptSuggestion
                    ...
                    onClick={() =>
                      handleSendMessage("Research industry trends")
                    }
                  >
                  ...
                  <PromptSuggestion
                    ...
                    onClick={() =>
                      handleSendMessage("Create strategic roadmap")
                    }
                  >
