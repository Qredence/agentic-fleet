...
export function useFastAPIChat({
  ...
  const setChatStatus = useCallback((nextStatus: ChatStatus) => {
  ...
  const handleSSEEvent = useCallback(
      ...
      switch (event.type) {
        ...
        case "workflow.event": {
          ...
          if (isPlanIndicator && event.text.includes(":")) {
            ...
            const planTitle =
              lines[0]?.replace(/.*plan:?/i, "").trim() || "Execution Plan";
    ...
    [messageState, approvalWorkflow, setChatStatus],
  );
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      const sendTask = (async () => {
        ...
        try {
          ...
          const payload = {
            model,
            input: content, // Backend expects simple string
            stream: true,
          };
          ...
          try {
            response = await fetchWithRetry(
              buildApiUrl(API_ENDPOINTS.RESPONSES),
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
      ...
      pendingSendRef.current = sendTask;
      try {
        await sendTask;
      } finally {
    ...
    [model, conversationId, messageState, handleSSEEvent, setChatStatus],
  );
