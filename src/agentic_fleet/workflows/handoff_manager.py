...
class HandoffManager:
    ...
    def __init__(
        self,
        dspy_supervisor: DSPySupervisor,
        get_compiled_supervisor: Callable[[], DSPySupervisor] | None = None,
    ):
        """Initialize HandoffManager.

        Args:
            dspy_supervisor: DSPy supervisor module for intelligent decisions
            get_compiled_supervisor: Optional provider that returns the compiled supervisor.
                When provided, handoff chains are invoked on the compiled supervisor;
        ...
        self.handoff_decision_module = (
            None if get_compiled_supervisor is not None else dspy.ChainOfThought(HandoffDecision)
        )
        self.handoff_protocol_module = (
            None if get_compiled_supervisor is not None else dspy.ChainOfThought(HandoffProtocol)
        )
        self.handoff_quality_module = (
            None
            if get_compiled_supervisor is not None
            else dspy.ChainOfThought(HandoffQualityAssessment)
        )
        self.handoff_history: list[HandoffContext] = []

    def _sup(self) -> DSPySupervisor:
        """Return preferred supervisor (compiled if provider is available)."""
    ...
    async def evaluate_handoff(
        ...
        if agent_states is None:
            agent_states = {name: "available" for name in available_agents}
        ...
        try:
            ...
            if hasattr(sup, "handoff_decision"):
                ...
                module = self.handoff_decision_module or dspy.ChainOfThought(HandoffDecision)
                decision = module(
                    current_agent=current_agent,
                    work_completed=work_completed,
                    remaining_work=remaining_work,
    ...
    async def create_handoff_package(
        ...
        try:
            ...
            if hasattr(sup, "handoff_protocol"):
                ...
                module = self.handoff_protocol_module or dspy.ChainOfThought(HandoffProtocol)
                protocol = module(
                    from_agent=from_agent,
                    to_agent=to_agent,
                    work_completed=work_completed,
