...
- `workflow/magentic_workflow.MagenticFleetWorkflowBuilder` loads workflow config, instantiates the
  ...
  `agent_framework.MagenticBuilder`. Imports upstream types from `agent_framework` (MagenticContext,
  OpenAIResponsesClient) rather than maintaining local copies.
...
- `api/responses/service.ResponseAggregator` adapts workflow events into OpenAI Responses-compatible
  payloads, powering both streaming (`text/event-stream`) and non-streaming responses. All event
  schemas live in `api/responses/schemas.py` (no duplicate models.py).
...
- Pydantic models in `models/` define API contracts. Update schemas first, then adjust routers or
  services.
- Legacy `core/` package has been removed; import directly from `agents`, `workflow`, `models`,
  `tools`, or `utils`.
