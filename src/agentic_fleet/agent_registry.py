...
def initialize_default_agents(app_manager, config_manager, user_session, defaults, env_config):
    ...
    code_executor = LocalCommandLineCodeExecutor(
        work_dir=workspace_dir, timeout=executor_settings["config"]["timeout"]
    )

    executor = CodeExecutorAgent(
        name="Executor", code_executor=code_executor, description=executor_settings["description"]
    )

    return {"websurfer": surfer, "filesurfer": file_surfer, "coder": coder, "executor": executor}
