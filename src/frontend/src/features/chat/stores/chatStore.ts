...
import { createErrorStep, createStatusStep } from "@/features/workflow/lib";
import type { ChatState } from "./types";
import {
  clearMessageContent,
  clearAllMessageContent,
  startNewMessage,
  addStepToLastMessage,
} from "./chat-event-handler";
import { startChatTransport, clearTransportTimeouts } from "./chat-transport";
...
const storeImpl = (set: ZustandSet, get: () => ChatState): ChatState => ({
  ...
  reset: () => {
    ...
    resetSSEClient();
    // Clear all accumulated message content to prevent memory leaks
    clearAllMessageContent();
    // Clear any active transport timeouts to prevent memory leaks
    clearTransportTimeouts();
