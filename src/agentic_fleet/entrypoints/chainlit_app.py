...
async def initialize_agent_for_profile(profile_name: str, client: object) -> object:
    ...
    try:
        ...
        logger.info(f"Initializing agent team for profile {profile_name}...")
        agents = initialize_default_agents(model_client=client)
        # For now, use the first agent as the team leader for streaming (update logic as needed)
...
def main():
    ...
    cmd = ["chainlit", "run", current_file, "--host", host, "--port", str(port), "--no-cache"]
