...
from .strategies import run_execution_phase
    ...
    async def handle_task(
        ...
        try:
            ...
            if (
                ...
                try:
                    # Note: perform_web_search_async needs to be available on supervisor or implemented here
                    # Assuming supervisor has it or we skip for now to keep it simple
                    pass
...
class RoutingExecutor(Executor):
    ...
    async def handle_analysis(
        ...
        try:
            ...
            self.context.latest_phase_status["routing"] = "fallback" if used_fallback else "success"
    ...
    async def handle_execution(
        ...
        try:
            ...
            if pipeline_profile == "light" or not enable_eval:
                progress_report = ProgressReport(action="complete", feedback="", used_fallback=True)
                used_fallback = True
            else:
...
class QualityExecutor(Executor):
    ...
    async def handle_progress(
        ...
        try:
            ...
            self.context.latest_phase_status["quality"] = "fallback" if used_fallback else "success"
            ...
            quality_report = QualityReport(
                score=5.0, missing="", improvements="", used_fallback=True
            )
