...
const storeImpl = (set: ZustandSet, get: () => ChatState): ChatState => ({
  ...
  loadConversations: async (reset = true) => {
    ...
    try {
      ...
      const convs = await api.listConversations({ limit, offset });
      ...
      set((state) => ({
        conversations: reset ? sorted : [...state.conversations, ...sorted],
        conversationsOffset: offset + sorted.length,
        hasMoreConversations: sorted.length >= limit,
        isInitializing: false,
      }));

      // Auto-select first conversation if none selected
      const { conversationId } = get();
      if (!conversationId && sorted.length > 0 && reset) {
        await get().selectConversation(sorted[0].id);
      } else if (!conversationId && sorted.length === 0 && reset) {
  ...
  loadMoreConversations: async () => {
    const { hasMoreConversations, isConversationsLoading } = get();
    if (!hasMoreConversations || isConversationsLoading) return;
